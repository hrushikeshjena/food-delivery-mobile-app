...
// //         Object.entries(route.params.cartItems).forEach(([item_id, quantity]) => {
// //           const item = {id: item_id, name: 'Item Name', price: 100} // Replace with actual item details
...
const Cart = ({navigation, route}) => {
  ...
  useEffect(() => {
    if (route.params?.cartItems) {
      const newCartItems = route.params.cartItems
      setCart(prevCart => {
        const updatedCart = [...prevCart]
        const updatedQuantities = {...quantity}

        Object.entries(newCartItems).forEach(([item_id, qty]) => {
          const item = {id: item_id, name: `Item ${item_id}`, price: 100} // Replace with actual item details

          const existingIndex = updatedCart.findIndex(
            cartItem => cartItem.id === item.id,
          )
          if (existingIndex > -1) {
            updatedQuantities[item.name] =
              (updatedQuantities[item.name] || 1) + qty
          } else {
            updatedCart.push(item)
            updatedQuantities[item.name] = qty
          }
        })
...
const PaymentFooter = ({
  ...
  <View style={styles.paymentFooter}>
    <RNPickerSelect
      ...
      style={pickerSelectStyles}
    />
    <Text style={styles.totalText}>Total: ₹{(total + 20).toFixed(2)}</Text>
...
// Styles (update these according to your needs)
const styles = StyleSheet.create({
  container: {flex: 1, padding: 16},
  header: {flexDirection: 'row', alignItems: 'center'},
  headerText: {fontSize: 24, fontWeight: 'bold', marginLeft: 10},
  deliveryContainer: {marginVertical: 16},
  delivery: {fontSize: 18},
  boldText: {fontWeight: 'bold'},
  itemsAddedSection: {marginVertical: 20},
  itemsAddedText: {fontSize: 20, fontWeight: 'bold'},
  emptyCartContainer: {alignItems: 'center', marginVertical: 30},
  emptyCartText: {fontSize: 18},
  cartItemContainer: {
    padding: 10,
    borderBottomWidth: 1,
  ...
  controlText: {fontSize: 20, marginHorizontal: 10},
  ...
  billingTitle: {fontSize: 20, fontWeight: 'bold'},
  billingInfo: {marginVertical: 10},
  billingRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    paddingVertical: 5,
